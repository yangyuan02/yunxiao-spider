var fs = require("fs")

var request = require("request")

var period = JSON.parse(fs.readFileSync('chapters/data2.txt','utf-8'))

var name = period.name //文件名字

var result = []

for(var key1 in period.children){
    for(var key2 in period.children[key1].children){
        if(key2=='book'){
            for(var key3 in period.children[key1].children[key2].children){
                for(var key4 in period.children[key1].children[key2].children[key3].children){
                    var obj = {
                        id : period.children[key1].children[key2].children[key3].children[key4].id,
                        name : name + '-' + key1 + '-教材-' + key3 + '-' + key4,
                        flag : 'book'
                    }
                    result.push(obj)
                }
            }
        }
        if(key2=='tree'){
            for(var key5 in period.children[key1].children[key2].children){
                var obj = {
                    id : period.children[key1].children[key2].children[key5].id,
                    name : name + '-' + key1 + '-知识树-' + key5,
                    flag : 'tree'
                }
                result.push(obj)
            }
        }
    }
}

var baseURL = 'http://tiku.yunxiao.com/api/question/'

var options = {
	url : '',//获取知识树
	headers : {
		'User-Agent' : 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36',
		'Host' : 'tiku.yunxiao.com',
		'Referer' : 'http://tiku.yunxiao.com/admin/makePaper/byQuestions',
		'Cookie' : 'tiku-user-info=%7B%22id%22%3A2437688687558656%2C%22name%22%3A%22%E5%88%98%E7%8E%89%22%2C%22schoolId%22%3A16696%2C%22schoolName%22%3A%22%E6%B1%9F%E8%8B%8F%E7%9C%81%E7%81%8C%E4%BA%91%E9%AB%98%E7%BA%A7%E4%B8%AD%E5%AD%A6%22%2C%22role%22%3A%22%E6%95%99%E5%B8%88%22%2C%22grade%22%3A%22%E9%AB%98%E4%B8%80%22%2C%22avatar%22%3A%22%22%2C%22exp%22%3A%222018-02-16T05%3A27%3A59.842Z%22%2C%22userId%22%3A2437688687558656%7D; tiku-session-id=; tiku-api-key=; tiku-theme=hfs-teacher; Hm_lvt_d9ce2e93fbe3e9d6109be3910c433855=1517562415,1517562419,1517562421,1517564731; Hm_lpvt_d9ce2e93fbe3e9d6109be3910c433855=1517564731',
		'Accept' : 'application/json, text/plain, */*'
	}
}
  
for(let i = 0;i < result.length ;i++){
    if(i == result.length){
        console.log(name + "爬取完成")
    }
    setTimeout(function(){
        options.url = result[i].flag == 'book' ? baseURL + 'book/chapters/?fold=undefined&book=' + result[i].id : baseURL + 'knowledge_tree/?fold=undefined&tree=' + result[i].id
        request(options, function(error,response,body){
            var info = JSON.parse(body)
            console.log("正在爬取--------" + result[i].name)
            fs.appendFile('chapters/xiao/' + result[i].name + '.txt' ,JSON.stringify(info,null,4),(err) =>{
                if(err){
                    console.log(err)
                }
            })
        });       
    },i*1000)
}